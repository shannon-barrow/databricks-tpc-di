...
CREATE OR REFRESH LIVE TABLE DimCustomerStg (
  ${DimCustomerStg.schema},
...
CREATE OR REFRESH LIVE TABLE DimCustomer (${DimCustomer.schema}
...
CREATE OR REFRESH LIVE TABLE DimAccount (${DimAccount.schema}
...
CREATE OR REFRESH LIVE TABLE FactMarketHistory (${FactMarketHistory.schema}
  --CONSTRAINT valid_security_symbol EXPECT (sk_securityid IS NOT NULL),
  --CONSTRAINT no_earnings_for_company EXPECT (PERatio IS NOT NULL)
  ) AS SELECT 
  s.sk_securityid,
  s.sk_companyid,
  sk_dateid,
...
CREATE OR REFRESH LIVE TABLE DimTrade (${DimTrade.schema}
...
--   CONSTRAINT invalid_trade_fee EXPECT (fee IS NULL OR fee <= tradeprice * quantity)
  ) AS SELECT
  trade.tradeid,
  sk_brokerid,
...
CREATE OR REFRESH LIVE TABLE FactHoldings (${FactHoldings.schema}
...
CREATE OR REFRESH LIVE TABLE FactCashBalances (${FactCashBalances.schema} 
...
CREATE OR REFRESH LIVE TABLE FactWatches (${FactWatches.schema}
  ...
  FROM (
    ...
    FROM ( 
      SELECT 
        ...
        wh.w_dts,
        if(w_action = 'ACTV', wh.batchid, null) batchid -- Need to keep the batchid of the date placed record, not date removed
      FROM (
