...
CREATE OR REFRESH LIVE TABLE FactMarketHistory (${factmarkethistoryschema}) AS SELECT 
  s.sk_securityid,
  s.sk_companyid,
  sk_dateid,
...
CREATE OR REFRESH TEMPORARY LIVE TABLE DimCustomerTemp AS 
SELECT
  customerid,
  _ActionTS,
...
CREATE OR REFRESH LIVE TABLE DimCustomer (${dimcustomerschema}) PARTITIONED BY (iscurrent) AS SELECT 
...
WHERE c.enddate != c.effectivedate
...
-- MAGIC * Code has to go get DimCustomer current batch, match the CURRENT batches customerid to the PREVIOUS CURRENT customerid, take the previous sk_customerid
...
CREATE OR REFRESH TEMPORARY LIVE TABLE AccountTemp AS SELECT * FROM ( 
  SELECT
    ...
    a.brokerid,
    a.status,
    nvl2(a.enddate, false, true) iscurrent,
    a.batchid,
    a.effectivedate,
...
CREATE OR REFRESH LIVE TABLE DimAccount (${dimaccountschema}) PARTITIONED BY (iscurrent) 
AS SELECT
...
WHERE a.enddate != a.effectivedate
...
CREATE OR REFRESH LIVE TABLE DimTrade (${dimtradeschema}) AS SELECT
  trade.tradeid,
  sk_brokerid,
...
LEFT JOIN LIVE.DimSecurity ds
  ON 
    ...
    AND date(trade.first_trade_ts) >= ds.effectivedate 
    AND date(trade.first_trade_ts) < ds.enddate
LEFT JOIN LIVE.DimAccount da
  ON 
    trade.t_ca_id = da.accountid 
    AND date(trade.first_trade_ts) >= da.effectivedate 
    AND date(trade.first_trade_ts) < da.enddate
...
CREATE OR REFRESH LIVE TABLE FactHoldings (${factholdingsschema}) 
AS SELECT 
  hh_h_t_id tradeid,
...
CREATE OR REFRESH LIVE TABLE FactWatches (${factwatchesschema}) 
AS SELECT
