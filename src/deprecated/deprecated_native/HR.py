...
with open(yaml_location, "r") as schemas:
  _schema = yaml.safe_load(schemas)['HR']['schema']

# COMMAND ----------

spark.read.csv(f'{files_directory}/{scale_factor}/Batch{batch_id}/HR.csv', schema=_schema, header=False, inferSchema=False).createOrReplaceTempView('v_hr')

# COMMAND ----------

spark.sql(f"""
  INSERT INTO {wh_db}.HR
  SELECT *
  FROM v_hr
""")
...
spark.sql(f"ANALYZE TABLE {wh_db}.HR COMPUTE STATISTICS")
