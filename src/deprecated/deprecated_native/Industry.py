...
with open(yaml_location, "r") as schemas:
  _schema = yaml.safe_load(schemas)['industry']['schema']

# COMMAND ----------

spark.read.csv(f'{files_directory}/{scale_factor}/Batch{batch_id}/Industry.txt', schema=_schema, sep='|', header=False, inferSchema=False).createOrReplaceTempView('v_industry')

# COMMAND ----------

spark.sql(f"""
  INSERT INTO {wh_db}.Industry
  SELECT *
  FROM v_industry
""")
...
spark.sql(f"ANALYZE TABLE {wh_db}.Industry COMPUTE STATISTICS")
