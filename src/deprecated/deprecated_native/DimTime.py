...
with open(yaml_location, "r") as schemas:
  _schema = yaml.safe_load(schemas)['dimTime']['schema']

# COMMAND ----------

spark.read.csv(f'{files_directory}/{scale_factor}/Batch{batch_id}/Time.txt', schema=_schema, sep='|', header=False, inferSchema=False).createOrReplaceTempView('v_Time')

# COMMAND ----------

spark.sql(f"""
  INSERT INTO {wh_db}.DimTime
  SELECT *
  FROM v_Time
""")
...
spark.sql(f"ANALYZE TABLE {wh_db}.DimTime COMPUTE STATISTICS")
