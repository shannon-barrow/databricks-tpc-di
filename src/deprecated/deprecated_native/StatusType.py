...
with open(yaml_location, "r") as schemas:
  _schema = yaml.safe_load(schemas)['statusType']['schema']

# COMMAND ----------

spark.read.csv(f'{files_directory}/{scale_factor}/Batch{batch_id}/StatusType.txt', schema=_schema, sep='|', header=False, inferSchema=False).createOrReplaceTempView('v_status')

# COMMAND ----------

spark.sql(f"""
  INSERT INTO {wh_db}.StatusType
  SELECT *
  FROM v_status
""")
...
spark.sql(f"ANALYZE TABLE {wh_db}.StatusType COMPUTE STATISTICS")
