...
with open(yaml_location, "r") as schemas:
  _schema = yaml.safe_load(schemas)['taxRate']['schema']

# COMMAND ----------

spark.read.csv(f'{files_directory}/{scale_factor}/Batch{batch_id}/TaxRate.txt', schema=_schema, sep='|', header=False, inferSchema=False).createOrReplaceTempView('v_tax')

# COMMAND ----------

spark.sql(f"""
  INSERT INTO {wh_db}.TaxRate
  SELECT *
  FROM v_tax
""")
...
spark.sql(f"ANALYZE TABLE {wh_db}.TaxRate COMPUTE STATISTICS")
