...
with open(yaml_location, "r") as schemas:
  _schema = yaml.safe_load(schemas)['dimDate']['schema']

# COMMAND ----------

spark.read.csv(f'{files_directory}/{scale_factor}/Batch{batch_id}/Date.txt', schema=_schema, sep='|', header=False, inferSchema=False).createOrReplaceTempView('v_DimDate')

# COMMAND ----------

spark.sql(f"""
INSERT INTO {wh_db}.DimDate
SELECT *
FROM v_DimDate
""")
...
spark.sql(f"ANALYZE TABLE {wh_db}.DimDate COMPUTE STATISTICS")
