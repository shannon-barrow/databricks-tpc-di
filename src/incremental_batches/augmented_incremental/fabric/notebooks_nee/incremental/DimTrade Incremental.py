# Fabric Spark NEE notebook — BATCH DimTrade (mirrors the dbt silver/dimtrade model).
# NEE is not streaming, so bronzetrade ACCUMULATES and this transform scopes to the batch
# with a date filter on the source (dbt: `where event_dt = batch_date`). Within the batch,
# max_by picks each trade's latest state; the MERGE closes trades that complete/cancel.
...
spark.sql(f"""
  with new_events as (
    ...
    select * from {tgt_db}.bronzetrade
...
notebookutils.notebook.exit(f"dimtrade_ok:{batch_date}")
