...
CLUSTER BY (tradeid);  -- modclust: was (sk_closedateid)
...
CLUSTER BY (customerid, symbol);  -- modclust: was (sk_dateid_dateremoved)
...
CREATE OR REFRESH MATERIALIZED VIEW factmarkethistorystg
CLUSTER BY (dm_s_symb)  -- modclust: added clustering on join key (was unclustered)
AS
select
  dm_s_symb,
  array_sort(
