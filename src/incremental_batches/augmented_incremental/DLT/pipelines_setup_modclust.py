...
shallow_tbls = [
  ...
  'dimbroker',
  'financial',
  # companyyeareps moved to CTAS-with-clustering below (modclust variant)
  'dimsecurity',
  'statustype',
  'dimcompany',
  'dimtime'
...
# modclust: companyyeareps gets CTAS with CLUSTER BY (sk_companyid). Source
# is partitioned by qtr_start_date (~6 GB / 950M rows at SF=20k); a shallow
# clone would inherit that partition layout, and Delta won't reorganize
# shared files into the new cluster key. CTAS rewrites with the new layout
# so the engineer's hypothesis (clustering on the MERGE join key)
# actually takes effect.
spark.sql(f"""
  CREATE OR REPLACE TABLE {catalog}.{tgt_db}.companyyeareps
  CLUSTER BY (sk_companyid)
  AS SELECT * FROM {catalog}.{staging_db}.companyyeareps
""")
spark.sql(f"ANALYZE TABLE {catalog}.{tgt_db}.companyyeareps COMPUTE STATISTICS FOR ALL COLUMNS")
