...
# ///
# Non-SS batch bronze ingest. simulate_filedrops wipes batches_dir (dbutils.fs.rm -r) and
# drops the day's files under a single `{batch_date}/` subfolder each batch, so exactly one
# `{Dataset}.txt` ever matches pathGlobFilter — read_files recurses into that lone date folder
# and reads the single file with no checkpoint / no file tracking (unlike the availableNow
# cloudFiles stream this replaces).
# All bronze tables ACCUMULATE via INSERT INTO (append); per-batch scoping happens DOWNSTREAM
# in each silver/gold via a `where <date_col> = batch_date` CTE. INSERT INTO ... SELECT
# preserves the setup-created CLUSTER BY + delta.dataSkippingNumIndexedCols=34.
...
spark.sql(f"""
  ...
  SELECT * FROM read_files(
    ...
    sep            => '|',
    schema         => '{schema}',
    pathGlobFilter => '{filename_format}'
  )
""")
