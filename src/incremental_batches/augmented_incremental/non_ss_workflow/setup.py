...
# MAGIC %md
# MAGIC # ONE-OFF TEST: strip NOT NULL + PK/FK constraints from the cloned tables
# MAGIC
# MAGIC Measures load-perf impact of dropping constraints — NOT NULL is enforced on
# MAGIC every write, so relaxing it can speed the per-batch MERGEs/INSERTs. Order
# MAGIC matters: drop FOREIGN KEYs first (they reference PKs), then PRIMARY KEYs,
# MAGIC then relax NOT NULL — otherwise a PK drop errors on a still-referencing FK.
# MAGIC Driven off information_schema so it adapts to whatever constraints the clones
# MAGIC inherited from staging. If this helps, fold the removal into the data-gen /
# MAGIC staging DDL instead of doing it here.

# COMMAND ----------

def _fq(t):
    return f"{catalog}.{tgt_db}.{t}"
...
for r in spark.sql(f"""
    ...
    try:
        ...
        print(f"[warn FK] {r.table_name}.{r.constraint_name}: {str(e)[:100]}")
...
for r in spark.sql(f"""
    ...
    try:
        ...
        print(f"[warn PK] {r.table_name}.{r.constraint_name}: {str(e)[:100]}")
...
for r in spark.sql(f"""
    ...
    try:
        ...
        print(f"[warn NN] {r.table_name}.{r.column_name}: {str(e)[:100]}")
