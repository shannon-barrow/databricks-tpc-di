...
def generate(spark: SparkSession, cfg, dicts: dict, dbutils) -> dict:
    ...
    # ManagerID: range 1 to 0.1*tableSize (DIGen starts manager IDs at 1, not 0) This means the top ~10% of employees are potential managers.
    ...
    # HR uses larger name dictionaries than CustomerMgmt: "hr_family_names" (Family-Names.dict, 13483 entries) -> employeefirstname "hr_given_names" (Given-Names.dict, 8608 entries) -> employeelastname (Note: DIGen's schema maps employeefirstname from Family-Names and employeelastname from Given-Names — the names are swapped vs intuition)
    ...
    # --- Broker allocation ---
    # 30% of employees become brokers (jobcode 314). Remaining employees get a random jobcode from OTHER_JOBCODES (9 values). DIGen never has null jobcodes — every employee has one.
    ...
    # Branch: 20-30 char random string using A-Za-z (DIGen pattern). Built by concatenating two MD5 hashes (64 hex chars total), translating hex digits to mixed-case letters, then taking a variable-length substring.
    ...
    # Extracts all employees with jobcode=314 (brokers) and assigns a sequential _idx (0-based) for downstream modules to reference via hash-based lookup. The customer module uses: broker_id = _brokers[hash(account) % broker_count] to assign each account to a valid broker.
    brokers = (final
    ...
    # Analytical estimate: 30% of employees become brokers (hash-based selection with ±0.01% variance). Exact count only computed when we need it for dynamic audit regeneration.
