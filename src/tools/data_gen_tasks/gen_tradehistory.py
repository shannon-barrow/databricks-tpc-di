...
# MAGIC
# MAGIC In augmented mode writes a Delta table at
# MAGIC ``tpcdi_raw_data.tradehistory{sf}`` instead.
...
if augmented_incremental and regenerate_data != "YES":
    fq = f"{catalog}.{raw_data_schema}.tradehistory{scale_factor}"
    if is_already_generated(spark, fq):
        print(f"[gen_tradehistory] {fq} already populated — skipping")
...
try:
    ...
    counts = trade_split.gen_tradehistory(spark, cfg, dbutils, base_df)
finally:
...
print(f"[gen_tradehistory] complete — {len(counts)} record_counts entries set")
