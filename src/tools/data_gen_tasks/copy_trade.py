...
# MAGIC
# MAGIC Copies the 4 Trade-family files staging → final across Batch1/2/3:
# MAGIC Trade.txt, TradeHistory.txt, CashTransaction.txt, HoldingHistory.txt.
# MAGIC One task because they all share the in-memory trade_df and
# MAGIC complete together. Augmented mode self-skips (gen_trade writes
# MAGIC Delta).
...
n = copy_dataset(cfg=ctx["cfg"], dbutils=dbutils,
                 filenames=["Trade.txt", "TradeHistory.txt",
                            "CashTransaction.txt", "HoldingHistory.txt"])
