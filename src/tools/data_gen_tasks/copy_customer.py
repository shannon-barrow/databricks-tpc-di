...
# MAGIC %md
# MAGIC # data_gen task: copy_customer (standard mode)
# MAGIC
# MAGIC Copies Customer.txt + Account.txt staging → final across Batch1/2/3.
# MAGIC Augmented mode skips this — gen_customer writes a Delta and produces
# MAGIC no `__staging` dirs, so this task self-skips per-batch when nothing
# MAGIC is staged.
...
augmented_incremental = dbutils.widgets.get("augmented_incremental").strip().lower() == "true"_nb_path = dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get()
...
n = copy_dataset(cfg=ctx["cfg"], dbutils=dbutils,
                 filenames=["Customer.txt", "Account.txt"])
print(f"[copy_customer] {n} files copied")
