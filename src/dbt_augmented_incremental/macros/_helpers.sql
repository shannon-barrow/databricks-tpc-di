...
{#   Bronze re-run guard                                                   #}
{# ----------------------------------------------------------------------- #}
{# Mirrors the Auto Loader checkpoint behaviour: on incremental runs, only
   ingest rows whose <date_col> is strictly greater than the latest already
   stored. Idempotent across same-day re-runs. #}
...
{# ----------------------------------------------------------------------- #}
{#   Static-staging table reference                                        #}
{# ----------------------------------------------------------------------- #}
{# The 19 dimensional + historical-fact tables produced by Stage 0 live in
   `{catalog}.tpcdi_incremental_staging_{sf}`. The dbt project does NOT
   re-ingest them — it CLONEs them via on-run-start (see clone_staging
   macro) into the run schema, then `ref()`s the cloned copies for
   downstream models. #}
{% macro staging_db() %}
