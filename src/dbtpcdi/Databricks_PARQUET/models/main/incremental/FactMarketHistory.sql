...
dailymarket as (
    SELECT
        *,
        1 batchid
    FROM parquet.`{{var("rawfilelocation")}}/sf={{var("scalefactor")}}/Batch1/DailyMarket*.parquet`
    UNION ALL
    SELECT
        * except(cdc_flag, cdc_dsn),
        int(substring(_metadata.file_path FROM (position('/Batch', _metadata.file_path) + 6) FOR 1)) batchid
    FROM parquet.`{{var("rawfilelocation")}}/sf={{var("scalefactor")}}/Batch{2,3}/DailyMarket.parquet`
),
markethistory as (
  SELECT
    dm.*,
